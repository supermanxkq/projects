				...
				<div class="container-fluid">
					...
					<div class="row-fluid">
						<div class="span12">
							...
							<form name=alipayment action="shop/shop!createOrder" method=post
									target="_blank" class="form-horizontal">
									<input type="hidden" name="goods.id" value="${goods.id}" />
										<div class="span12">
											 <input type="hidden" size="30" name="order.out_trade_no" value="${out_trade_no}" />
											<p>
												商品名称：${goods.name }<input name="order.subject" value="${goods.name }" type="hidden" ></input>
											</p>
												<p>
												价格：
												<font style="color: #C00;">￥${goods.price }.0元</font >
												<input type="hidden" name="order.price" value="${goods.price }" ></input>
											</p>
												<img src="${goods.imgsrc}"/>
												<br/>
												<input type="submit" class="btn btn-success btn-lg" value="立即购买"/>
												<p>
												${goods.description }
												</p>
										</div>
							...
							<div id="SOHUCS" sid="${goods.id}"></div>
								</div>
							</div>
						</div>
					</div>
