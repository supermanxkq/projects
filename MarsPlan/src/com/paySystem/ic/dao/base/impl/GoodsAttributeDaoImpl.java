...
public class GoodsAttributeDaoImpl extends DaoSupport<GoodsAttribute> implements
		GoodsAttributeDao {
	 ...
	 */
	private QueryResult<GoodsAttributeDTO> dtoResult2beanResult(
			QueryResult<GoodsAttribute> list) {
	 ...
	 */
	public List<GoodsAttributeDTO> getAttributeByFamilyId(Integer familyId)
			throws Exception {
		Query query = em.createNativeQuery("select a.attrId,a.attrName from b_attribute as a where a.attrId in (select fa.attrId from b_familyattrrela as fa where fa.familyId=?)");
	 ...
	 */
	public List<GoodsAttributeDTO> getParnetAttributeByFamilyId(Integer familyId)
			throws Exception {
