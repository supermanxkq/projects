...
public class MerchantsAction extends BaseAction {
	...
	public String list() throws Exception {
		...
		this.getRequest().setAttribute("condValues",
				OptionsValue.MERCHANT_STATUS);// 状态
	...
	public String jsonPageList() throws Exception {
		...
		QueryResult queryResult = merchantsService.queryMerByCond((merchantsDTO
				.getPage() - 1)
				* pageNum, pageNum, merchantsDTO, orderby);
		...
		for (int i = 0; i < list.size(); i++) {
			...
			strings.add(Utils.getOptionsIntegerName(
					OptionsValue.MERCHANT_STATUS, merchants.getStatus()));
			strings.add(DateTimeTool.dateFormat("", merchants.getUpdateTime()));
			String operation = "";

			if (merchants.getStatus() != 9) {
				if (Utils.checkPermission("sy-1201-03")) {
					operation += "<a href=base/merchants!editUI?merchantsDTO.merId="
							+ merchants.getMerId()
							+ " title='修改'>"
							+ Globals.IMG_EDIT + "</a>&nbsp;";
				}
				if (Utils.checkPermission("sy-1201-04")) {
					operation += "<a href=javascript:deleteData('base/merchants!delete','"
							+ merchants.getMerId()
							+ "') title='删除'>"
				...
				if (Utils.checkPermission("sy-1201-01")) {
					operation += "<a href=base/merchants!checkUI?merchantsDTO.merId="
							+ merchants.getMerId()
							+ " title='查看'>"
							+ Globals.IMG_VIEW + "</a>&nbsp;";
				}
		...
		PageView pageView = new PageView(merchantsDTO.getPage(), queryResult
				.getTotalrecord());
	...
	public String mercJsonPageList() throws Exception {
		...
		for (int i = 0; i < list.size(); i++) {
			...
			strings.add(Utils.getOptionsIntegerName(
					OptionsValue.MERCHANT_STATUS, merchants.getStatus()));
			strings.add(DateTimeTool.dateFormat("", merchants.getUpdateTime()));
			String operation = "<a href=javascript:secMerc('"
					+ merchants.getMerId() + "','"
					+ merchants.getOrgans().getOrganId() + "','"
					+ merchants.getMerName() + "','"
	...
	public String addUI() {
		...
		merchantsDTO.setIsSalePointStr(Utils.getSelectHtml(
				OptionsValue.VISIBLE_STATUS, "merchantsDTO.isSalePoints",
				"isSalePoints", false));
		this.getRequest().setAttribute("organsValues",
				organsService.getOption());// 机构
		...
		this.getRequest().setAttribute("settlementValues",
				OptionsValue.SETTLEMENT_STATUS);// 结算方式
		this.getRequest().setAttribute("areaValues", OptionsValue.AREA_VALUE);
		this.getRequest().setAttribute("settelCountWayValues",
				OptionsValue.SETTLE_COUNT_WAY);
		this.getRequest().setAttribute("convIsOrNot",
	...
	public String addSave() throws Exception {
		...
		if (merchantsService.validateName(merchantsDTO.getMerName(),
				merchantsDTO.getMerId())) {
		...
		functionsService
				.saveFunction("商户管理", 1, "增加商户：" + merchants.getMerId());
	...
	public String editUI() throws Exception {
		...
		if (merchants != null) {
			...
			this.getRequest().setAttribute("settlementValues",
					OptionsValue.SETTLEMENT_STATUS);// 结算方式
			this.getRequest().setAttribute("areaValues",
					OptionsValue.AREA_VALUE);
			this.getRequest().setAttribute("settelCountWayValues",
					OptionsValue.SETTLE_COUNT_WAY);
			this.getRequest().setAttribute("convIsOrNot",
					OptionsValue.VISIBLE_STATUS_CONV);
	...
	public String editSave() {
		...
		try {
			...
			if (dto.getFlag()) {
				functionsService.saveFunction("商户管理", 2, "修改商户："
						+ merchantsDTO.getMerId());
				this.getRequest().setAttribute("result",
						this.getText("operation.success.notice"));
				this.getRequest().setAttribute("url", "base/merchants!list");
	...
	public String checkUI() {
		...
		if (merchants != null) {
			...
			/** 添加收单关系信息 */
			SingleRelation singleRelation = sinService
					.findByMerIdBinId(merchants.getMerId());
			if (singleRelation != null) {
				...
				merchantsDTO.setPlatformRate(singleRelation
						.getServPlatformRatio());// 服务平台比率
				...
				merchantsDTO.setSettelCountWay(singleRelation
						.getCountSettType());
			}

			/** 允许消费的卡BIN信息 */
			List<MerchantsDTO> cardBinDtos = new ArrayList<MerchantsDTO>();
			List<OptionsString> list = cardBinService
					.getOptionByOrganId(organId);
			/*
	...
	public String delete() {
		...
		try {
			...
			functionsService.saveFunction("商户管理", 3, "删除商户："
					+ merchants.getMerId());
	...
	public String preMerchantsHelpList() throws Exception {
		...
		QueryResult<Merchants> queryResult = merchantsService.preMerHelpList(
				(merchantsDTO.getPage() - 1) * pageNum, pageNum, merchantsDTO,
				orderby);
		...
		for (int i = 0; i < list.size(); i++) {
			...
			strings.add(Utils.getOptionsIntegerName(
					OptionsValue.MERCHANT_STATUS, merchants.getStatus()));
			strings.add(DateTimeTool.dateFormat("", merchants.getUpdateTime()));
			String operation = "<a href=javascript:" + "secPreMer('"
					+ merchants.getMerId() + "'," + "'"
					+ merchants.getMerName() + "')"
					+ " title='选择'>[选择]</a>&nbsp;";
			strings.add(operation);
	...
	public String orgJsonPageList() throws Exception {
		...
		for (int i = 0; i < list.size(); i++) {
			...
			strings.add(Utils.getString(merchants.getMerId()));
			strings.add(Utils.getString(merchants.getMerName()));
			String operation = "<a href=javascript:secMer('"
					+ merchants.getMerId() + "','" + merchants.getMerName()
					+ "') title='选择'>[选择]</a>&nbsp;";
			strings.add(operation);
			lists.add(strings);
		}
	...
	private MerchantsDTO getMerDto(Merchants merchants,
		...
		if (merchants.getPreMerId() != null && merchants.getPreMerId() != ""
				&& (!"".equals(merchants.getPreMerId()))) {
		...
		merchantsDTO.setAgentDiscRate((us.getUserLevel() == 2 && merchants
				.getAgentDiscRate() == null) ? new BigDecimal(0) : merchants
				.getAgentDiscRate());
		merchantsDTO.setTranLimitSign(merchants.getTranLimitSign());
		merchantsDTO.setRevorkReason(Utils.getString(merchants
				.getRevorkReason()));
		merchantsDTO
				.setConNo(Utils.getString((us.getUserLevel() == 2 && merchants
						.getConNo() == null) ? " 暂无信息" : merchants.getConNo()));
		...
		merchantsDTO.setConPerTeleNo(Utils.getString(merchants
				.getConPerTeleNo()));
		merchantsDTO.setBankName(Utils.getString(merchants.getBankName()));
		merchantsDTO.setBankAccountNo(Utils.getString(merchants
				.getBankAccountNo()));
