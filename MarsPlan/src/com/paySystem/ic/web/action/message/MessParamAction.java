...
public class MessParamAction extends BaseAction{
	...
	public String jsonPageList() throws Exception{
		...
		List<MessageDTO> messList = messFeeParamService.queryAll(
				(messDto.getPage() - 1) * pageNum, pageNum, messDto,
				orderBy);
		...
		for (int i = 0; i < messList.size(); i++) {
			...
			if(us.getUserLevel()!=2){				
			  if(Utils.checkPermission("sy-1601-03")){
				operation += "<a href=message/messageparam!editUI?messDto.mfpId="+messageDTO.getMfpId()+" title='修改'>"+Globals.IMG_EDIT+"</a>&nbsp;";
			  }
			  if(messageDTO.getUseSign()!=9){
				operation += "<a href=javascript:deleteData('message/messageparam!delete','"+messageDTO.getMfpId()+"') title='删除'>"+Globals.IMG_DELETE+"</a>&nbsp;";
			  }
			}
		...
		PageView pageView = new PageView(messDto.getPage(), messList
				.size());
    ...
    public String addSave(){
    	...
    	if(!messFeeParamService.queryMessName(messDto.getMessName())){
			this.getRequest().setAttribute("result","该参数名称已存在!");
			this.getRequest().setAttribute("url","message/messageparam!addUI");
			return ERROR;			
	    }
    ...
    public String mfpjsonPageList() throws Exception{
		...
		for (int i = 0; i < messList.size(); i++) {
			...
			strings.add(NumberUtil.numberFormat("", messageDTO.getMessFee()));
			strings.add(Utils.getOptionsIntegerName(OptionsValue.STATE_STATUS,messageDTO.getUseSign()));
			String operation = "<a href=javascript:secMfp('"+messageDTO.getMfpId() + "','"+messageDTO.getMessName()+"'); title='选择'>[选择]</a>&nbsp;";
			strings.add(operation);			
			lists.add(strings);
		}
