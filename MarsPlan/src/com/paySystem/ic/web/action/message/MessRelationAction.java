...
public class MessRelationAction extends BaseAction {
	...
	public String list(){
		...
		if(us.getUserLevel()!=2){
			...
			messDto.setBeginTime(DateTimeTool.dateFormat("yyyy-MM-dd",
	...
	public String jsonPageList() throws Exception{
		...
		List<MessageDTO> messList = messParamRelationService.queryAll(
		...
		for (int i = 0; i < messList.size(); i++) {
			...
			String operation = "";				
			if(Utils.checkPermission("sy-1602-03")){
				//判断权限在相关操作栏显示相关操作
			    operation += "<a href=message/messagerelation!editUI?messDto.mprId="+messageDTO.getMprId()+" title='修改'>"+Globals.IMG_EDIT+"</a>&nbsp;";
			}
			if(Utils.checkPermission("sy-1602-04")&&messageDTO.getState()!=9){
				//判断权限在相关操作栏显示相关操作，若状态为删除状态则不显示
				operation += "<a href=javascript:deleteData('message/messagerelation!delete','"+messageDTO.getMprId()+"') title='删除'>"+Globals.IMG_DELETE+"</a>&nbsp;";
			}			
    ...
    public String addSave(){
    	...
    	if(!messParamRelationService.queryRelation(messDto.getOrgMerId())){
    		//判断该机构/商户是否已存在
			this.getRequest().setAttribute("result","该参数关系已存在!");
