...
function switchAll() {
	for (var i = 0; i < document.getElementsByName("id").length; i++) {
		document.getElementsByName("id")[i].checked = !document
				.getElementsByName("id")[i].checked;
	}
}
...
function switchAllByName(name) {
	for (var i = 0; i < document.getElementsByName(name).length; i++) {
		document.getElementsByName(name)[i].checked = !document
				.getElementsByName(name)[i].checked;
	}
}
...
function ajaxData(dataUrl, params) {
	...
	$.ajax({
				...
				// first.nextAll().remove();
				// var tr = $("<tr></tr>");
				// var html = "<td colspan='"+thLength+"'><img
				// src='images/loading.gif' /> <b>数据加载中 ....</b></td>";
				// tr.html(html);
				// tb.append(tr);
				// },
...
function ajaxDatas(dataUrl, params, tbId, pageNav) {
	...
	$.ajax({
		...
		beforeSend : function() {
			...
			var html = "<td colspan='"
					+ thLength
					+ "'><img src='images/loading.gif' /> <b>数据加载中 ....</b></td>";
			tr.html(html);
			tb.append(tr);
		},
		success : function(data, textStatus) {
			...
			if (list.length > 0) {
				...
				var tr = $("<tr></tr>");
				var html = "<td colspan='" + thLength + "' >当前箱子没有未完成任务！</td>";
				tr.html(html);
				tb.append(tr);
			}
		}
...
var deleteData = function(url, id) {
	...
	$.ajax({
				...
				beforeSend : function() {
					/** 处理gif */
					$("#data_loading").removeClass()
							.addClass("animated flipInX").show();
				...
				success : function(data, textStatus) {
					if (data.ajaxResult == "ajaxsuccess") {
						...
						$(document).ready(function() {
									...
									$.messager.show('', '恭喜！任务已完成！o(≧v≦)o', 4000);
								});
						query($("#currPage").text());
...
function isIDno(strIDno) {
	/***************************************************************************
	 * if (isIDno(sfzNo.value) == false) { alert("不存在此身份证"); sfzNo.focus();
	 * return false; }
	 **************************************************************************/
	var aCity = {
		11 : "北京",
...
function verifyUKey(isVerify) {
	if (isVerify) {
		...
		try {
			...
			// 读取锁的唯一ID
			$("#keyID").val(toHex(aObject.GetID_1(DevicePath))
					+ toHex(aObject.GetID_2(DevicePath)));

			if (aObject.LastError != 0) {
				window.alert("Err to GetID,ErrCode is:"
						+ aObject.LastError.toString());
				return false;
			}
			// 这里返回对随机数的HASH结果
			$("#return_EncData").val(aObject.EncString(document
							.getElementById('rnd').value, DevicePath));

			if (aObject.LastError != 0) {
				window.alert("Err to StrEnc,ErrCode is:"
						+ aObject.LastError.toString());
				return false;
			}
			return true;
...
function getCipherCode() {
	...
	try {
		...
		// 读取锁的唯一ID
		$("#jmKeyID").val(toHex(aObject.GetID_1(DevicePath))
				+ toHex(aObject.GetID_2(DevicePath)));
		 ...
		 * 
		 * var cipherCode = aObject.YReadString(100,mylen, "ffffffff",
		 * "ffffffff", DevicePath); if( aObject.LastError!= 0 ){
		 * window.alert("Err to GetUserName,ErrCode
		 * is:"+aObject.LastError.toString()); return ; }
		 * alert(aObject.LastError+"-"+cipherCode); if(cipherCode==""){
		 * alert("UKEY未设置密钥"); return false; }
		 **********************************************************************/
		...
		// 再读取相应长度的字符串
		cipherCode = aObject.YReadString(addr + 1, nlen, "ffffffff",
				"ffffffff", DevicePath);
		if (aObject.LastError != 0) {
