...
public class PromotionDaoImpl extends DaoSupport<Promotion> implements PromotionDao {
	...
	public Integer savePromotion(PromotionDTO promotionDTO) {
		...
		BeanUtils.copyProperties(promotionDTO, promotion,new String[]{"beginTime","endTime","proImg","descr"});
		try {
		...
		if(promotionDTO.getProid()==null){
			...
			Promotion oldPromotion = this.find(promotionDTO.getProid());
		     ...
		     if(oldPromotion!=null){
				...
				if(promotionDTO.getDeleteProImg() == 0 && StringUtils.isBlank(promotionDTO.getProImgFileName())) {
	...
	public List<PromotionDTO> findOnlinePromotionPlatform(String merId,String userName) throws Exception {
		...
		List<Object[]> onlineProList = em.createNativeQuery("select o2.proId,o2.proname from B_Promotion o2 left outer join B_MerApplyRecord o1   on o1.proId=o2.proId where (o2.operatorId=? and  o2.status not in (5,6,9))" 
															+"or (o1.merid=? and  o1.status=2  and  o2.status not in (5,6,9))").setParameter(1, userName).setParameter(2, merId).getResultList();
	 ...
	 *@Thorws:
	 */
	private QueryResult<PromotionDTO> beanResult2dtoResult(QueryResult<Promotion> result){
		
		
		QueryResult<PromotionDTO> dtoResult 
		                            = new QueryResult<PromotionDTO>();
		List<PromotionDTO> dtoList
		                            = new ArrayList<PromotionDTO>();
		...
		if(result!=null){
			...
			for (Promotion promotion : result.getResultlist()){
				...
				BeanUtils.copyProperties(promotion,promotionDTO,new String[]{"proImg","descr"});
	...
	/**
	 *@OverRiddenMethod：@see com.paySystem.ic.dao.buss.PromotionDao#findGoodsCountByProid(int)
	...
	public long findGoodsCountByProid(int proid) {
		String sql = "select count(*) from B_GoodsInfo goods where goods.goodsSta=0 and  goods.proid="+proid;
	...
	/**
	 *@OverRiddenMethod：@see com.paySystem.ic.dao.buss.PromotionDao#openRecruit(java.lang.Integer)
	...
	/**
	 *@OverRiddenMethod：@see com.paySystem.ic.dao.buss.PromotionDao#stopRecruit(java.lang.Integer)
	...
	/**
	 *@OverRiddenMethod：@see com.paySystem.ic.dao.buss.PromotionDao#stopPromotion(java.lang.Integer)
	...
	/**
	 *@OverRiddenMethod：@see com.paySystem.ic.dao.buss.PromotionDao#beginPromotion(java.lang.Integer)
	...
	public QueryResult<List> queryApplyRecordByCond(int firstPage, int pageNum,
		...
		if(merApplyRecordDTO.getMerid()!=null && !merApplyRecordDTO.getMerid().equals("")){
