...
public class AnnountDAOImpl extends DaoSupport<Annount> implements AnnountDAO{
	...
	/**
	 *@OverRiddenMethod：@see com.paySystem.ic.dao.marketing.annountDAO#queryResult(int, int, com.paySystem.ic.web.dto.marketing.AnnountDTO, java.util.LinkedHashMap)
	 *@MethodName:queryResult
	 *@Description:查询方法
	 *@param firstindex 第一个索引
	 *@param maxresult 最后一个索引
	 *@param annountDTO
	 *@param orderby
	 *@return
	 *@throws Exception
	...
	public QueryResult<AnnountDTO> queryResult(int firstindex, int maxresult,
		...
		if(!"-1".equals(annountDTO.getStatus())&&-1!=annountDTO.getStatus()){
		...
		if (StringUtils.isNotBlank(annountDTO.getCreateTime())) {
			sql
					.append(" and str_to_date(o.createTime,'%Y-%m-%d') "
							+ ">=str_to_date('"
							+ annountDTO.getCreateTime()
							+ "','%Y-%m-%d')");
		}
	...
	*/
	private List<AnnountDTO> getAnnountDTOList(List<Annount> list) {
		...
		for (int i = 0; i < list.size(); i++) {
			...
			annountDTO.setReportTime(DateTimeTool.dateFormat("yyyy-MM-dd HH:mm:ss", annount.getReportTime()));
	...
	/**
	 *@OverRiddenMethod：@see com.paySystem.ic.dao.marketing.AnnountDAO#addSaveAnnount(com.paySystem.ic.web.dto.marketing.AnnountDTO)
	...
	*/
	private Annount getAnnount(AnnountDTO annountDTO) {
		...
		if (annountDTO.getReportTime() != null) {
			annount.setReportTime(DateTimeTool.dateFormat("yyyy-MM-dd HH:mm:ss", annountDTO.getReportTime()));
		}
		...
		{
			annount.setCreateTime(DateTimeTool.dateFormat("yyyy-MM-dd HH:mm:ss", annountDTO.getCreateTime()));
		}
		return annount;
	}
	/**
	 *@OverRiddenMethod：@see com.paySystem.ic.dao.marketing.AnnountDAO#findAnnounTitle(java.lang.String)
	...
	/**
	 *@OverRiddenMethod：@see com.paySystem.ic.dao.marketing.AnnountDAO#getAnnountDTO(com.paySystem.ic.bean.Marketing.Annount)
	...
	/**
	 *@OverRiddenMethod：@see com.paySystem.ic.dao.marketing.AnnountDAO#updateAnnount(com.paySystem.ic.web.dto.marketing.AnnountDTO)
