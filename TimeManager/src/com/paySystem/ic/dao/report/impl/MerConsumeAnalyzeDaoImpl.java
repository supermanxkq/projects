...
public class MerConsumeAnalyzeDaoImpl extends DaoSupport<Trades> implements
		MerConsumeAnalyzeDao {
	@Resource
	OrgansDao organsDao;
	...
	public List<MerConsumeAnalyzeDTO> queryAll(int page, int pageNum,
		...
		StringBuilder sb = new StringBuilder();
			...
			if (merConsumeAnalyzeDTo.getStatus()==1) {
				sb.append("select t.merid,b.mername,to_char(t.placedtime,'yyyy-mm') as 对应时间," +
						"to_char(t.placedtime,'mm') as 月份,sum(t.tradeamt) as 消费金额 ," +
						"count(t.merid) as counts");
				 
			}
			if (merConsumeAnalyzeDTo.getStatus()==2) {
				sb.append("select t.merid,b.mername,to_char(t.placedtime,'yyyy-mm-dd') as 对应时间," +
						"to_char(t.placedtime,'dd') as 天,sum(t.tradeamt) as 消费金额 ," +
						"count(t.merid) as counts ");
		...
		try {
			merConsumeList = em.createNativeQuery(
