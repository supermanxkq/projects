...
public class advertmaAction extends BaseAction {
	...
	@Resource AdvertMaService advertmasercvice;
	@Resource FunctionsService functionservice;
	...
	*/
	public String list(){
		this.getRequest().setAttribute("mediaTypeList",OptionsValue.Advertma_mediaType);
		return "list";
	}
	/**
	*@Title:jsonPageList
	...
	*/
    ...
    public String jsonPageList() {
		...
		try {
			result = advertmasercvice.queryResult((advertmaDTO.getPage()-1)*pageNum, pageNum, advertmaDTO, orderby);
		} catch (Exception e) {
			e.printStackTrace();
		}
        ...
        for (int i = 0; i < listDto.size(); i++) {
            ...
            strings.add(Utils.getOptionsIntegerName(OptionsValue.Advertma_location,
            		dto.getPositionTypeId()));
            strings.add(Utils.getOptionsIntegerName(OptionsValue.Advertma_mediaType,
            		dto.getMediaType()));
            dto.setBeginTime(DateTimeTool.dateFormat("yyyy-MM-dd HH:mm:ss",
    				DateTimeTool.dateFormat("yyyy-MM-dd HH:mm:ss", dto.getBeginTime())));
            strings.add(dto.getBeginTime());
            dto.setEndTime(DateTimeTool.dateFormat("yyyy-MM-dd HH:mm:ss",
    				DateTimeTool.dateFormat("yyyy-MM-dd HH:mm:ss", dto.getEndTime())));
            strings.add(dto.getEndTime());
            String operation = "";
            if(dto.getMediaType()!=9)
            {

	            if (Utils.checkPermission("sy-7201-03")) {
	                operation += "<a href =marketing/advertma!editUI?advertmaDTO.advertId=" + dto.getAdvertId()
	                        + " title='修改'>" + Globals.IMG_EDIT + "</a>&nbsp;";
	            }
	            /**判断权限在相关操作栏显示相关操作，若状态为删除状态则不显示*/
	            if (Utils.checkPermission("sy-7201-04")) {
	                operation += "<a href=javascript:deleteData('marketing/advertma!delete','" + dto.getAdvertId()
	                        + "') title='删除'>" + Globals.IMG_DELETE + "</a>&nbsp;";
	            }
	            if (Utils.checkPermission("sy-7201-01")) {
	                operation += "<a href =marketing/advertma!checkUI?advertmaDTO.advertId=" + dto.getAdvertId()
        ...
        PageView pageView = new PageView(advertmaDTO.getPage(), result.getTotalrecord());
	...
	*/
	public String addUI() {
		...
		this.getRequest().setAttribute("mediaTypeList",OptionsValue.Advertma_addmediaType);
		this.getRequest().setAttribute("mediaLocationList",OptionsValue.Advertma_location);
		return "set";
	}
	/**
	*@Title:addSave
	...
	*/
	public String addSave() throws Exception {
		...
		/** 添加功能日志信息 */
		functionservice.saveFunction("全站广告管理", 3, "添加广告"+advertmaDTO.getAdvertId());
	...
	*/
	public String checkadvertName() {
		...
		/** 设置返回标志信息 */
		dto.setFlag(advertmasercvice.findAdvertMaName(advertName, method, advertId));
