...
public class MemGroupsAction extends BaseAction {
	...
	public String jsonPageList() throws Exception {
		...
		if (StringUtils.isNotBlank(this.getOrderProperty())
			...
			orderby.put("groupId", "ASC");
		}
		QueryResult<MemGroups> queryResult = memGroupsServiceImpl.queryResult(
				(memGroupsDTO.getPage() - 1) * pageNum, pageNum, memGroupsDTO,
				orderby);
		...
		for (int i = 0; i < list.size(); i++) {
			...
			strings.add(DateTimeTool.dateFormat("yyyy-MM-dd HH:mm:ss", memGroups
					.getCreateTime()));
			...
			if (Utils.checkPermission("sy-1305-01")) {
				operation += "<a href=member/memGroups!checkUI?memGroupsDTO.groupId="
						+ memGroups.getGroupId()
						+ " title='查看'>"
						+ Globals.IMG_VIEW + "</a>&nbsp;";
			}
			if (Utils.checkPermission("sy-1305-03")) {
				operation += "<a href=member/memGroups!editUI?memGroupsDTO.groupId="
						+ memGroups.getGroupId()
						+ " title='修改'>"
						+ Globals.IMG_EDIT + "</a>&nbsp;";
			}
			if (Utils.checkPermission("sy-1305-04")) {
				operation += "<a href=javascript:deleteData('member/memGroups!delete','"
						+ memGroups.getGroupId()
						+ "') title='刪除'>"
		...
		PageView pageView = new PageView(memGroupsDTO.getPage(), queryResult
				.getTotalrecord());
...
//		}
		...
		groupList = memGroupsServiceImpl.getMemberByGroupId(memGroupsDTO
				.getGroupId());
	...
	public String editSave() {
		...
		try {
			...
			if (dto.getFlag()) {
				functionsService.saveFunction("会员群组管理", 2, "修改会员群组："
	...
	public String delete() {
		try {
			// ID 值得是
			MemGroups memGroups = memGroupsServiceImpl.find(Integer
					.valueOf(this.getId()));
			memGroups.setStatus(9);
			memGroupsServiceImpl.update(memGroups);
			functionsService.saveFunction("会员群组管理", 3, "删除会员群组："
					+ memGroups.getGroupId().toString());
	...
	public String addSave() {
		...
		functionsService.saveFunction("会员群组管理", 1, "增加群组会员："
				+ memGroupsDTO.getMemId());
		this.getRequest().setAttribute("result",
				this.getText("operation.success.notice"));
		this.getRequest().setAttribute("url", "member/memGroups!list");
		return SUCCESS;
	...
	public String checkUI() throws Exception {
		...
		List<MemberGroupsDetail> list = memberGroupsDetailService
				.findByGroupId(memGroupsDTO.getGroupId());
		for (int i = 0; i < list.size(); i++) {
		...
		if (memGroupsDTO != null) {
			...
			if (memGroupsDTO != null) {
				...
				memGroupsDTO.setGroupDesc(Utils.getString(memGroups
						.getGroupDesc()));
				memGroupsDTO.setUserName(Utils.getString(memGroups
						.getUserName()));
			...
			this.getRequest().setAttribute("areaIdValues",
					OptionsValue.AREA_VALUE);// 地区类型
			return "look";
		}
	...
	public String queryAllList() {
		...
		List<OptionsString> listt = memberService.findMemOptionString(memDto,memGroupsDTO.getGroupId());
	...
	public void setMemberGroupsDetailDTO(
			MemberGroupsDetailDTO memberGroupsDetailDTO) {
