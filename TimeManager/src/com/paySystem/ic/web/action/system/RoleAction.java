...
public class RoleAction extends BaseAction {
	...
	public String jsonPageList() throws Exception {
		...
		for(int i = 0;i <list.size();i++){
			...
			strings.add(Utils.getOptionsIntegerName(OptionsValue.STATE_STATUS, role.getStatus()));
			String operation = "";
			if(role.getStatus()!=9){
					...
					for (Module m : midmodule.getChildModules()) {
						...
						for (Privilege privilege : m.getPrivileges()) {
							if(privilege.getPrivilegeLevel()!=null&&privilege.getPrivilegeLevel()==0){
								if(roleDTO.getOrganId().equals(Globals.MAIN_ORGANID)||roleDTO.getOrganId().equals(Globals.MAIN_ORGANID_1)){
