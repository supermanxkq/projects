        ...
        success : function(data, textStatus) {  
        	...
        	if (list.length>0){
        		for(var i=0;i<list.length;i++) {
        			var div = $("<div class='row-fluid blog '></div>");
        			var html = "";
        				html += 
        						"<div class='span12'>" +
        						"<p>"+list[i][3]+"</p>" +
        						"<h2><a href=article/article!checkUI?columnCode=1&articleDTO.articleId="+list[i][0]+">"+list[i][1]+"</a></h2>"+
        						"<p>作者:"+list[i][4]+"&nbsp;&nbsp;分类："+list[i][5]+"&nbsp;&nbsp;阅读("+list[i][7]+")  <span class='ds-thread-count' data-thread-key='"+list[i][0]+"' data-count-type='comments'></span></p>"+
        						"<p>"+list[i][6]+"</p>";
        						//如果属于当前用户
        						if(list[i][9]=="1"){
        							html+="<ul style='margin-left:700px;'><li>"+
        	                            "<a href=article/article!editUI?articleDTO.articleId="+list[i][0]+"><i class='icon-trash'></i> 修改</a></li><li>"+
        	                            "<a href=javascript:deleteData('article/article!delete','"+ list[i][0]+ "')><i class='icon-remove'></i>删除</a></li></ul>";
        						}
							html+="<br/><br/><hr style='box-sizing: border-box; margin: 5px 0px 20px; font-family: 'Microsoft Yahei', 'Helvetica Neue', Helvetica, Arial, sans-serif; font-weight: normal; line-height: 35px; color: rgb(85, 85, 85); text-rendering: optimizeLegibility; font-size: 16px; border-bottom-color: rgb(238, 238, 238); border-bottom-width: 1px; border-bottom-style: solid; padding-bottom: 5px; white-space: normal;' />";
        			div.html(html);
        			tb.append(div);
            	}
        	} else {
...
//异步加载数据
function ajaxDataForPage(dataUrl,params){
  $.ajax( {   
      url : dataUrl, 
      data : params,   
      dataType : "json",
      ...
      success : function(data, textStatus) {
			...
			for ( var i = 0; i < data.list.length; i++) {
				 $("#content").append("留言者："+data.list[i].leaveMessageName+"&nbsp;&nbsp;留言内容："+data.list[i].leaveMessageContent+"</br><hr>"); 
			}
			$("#pageNav").html(data.pagehtml);		 
      }
  });
