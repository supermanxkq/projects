...
public class MerchantsDaoImpl extends DaoSupport<Merchants> implements MerchantsDao {
	...
	/*	String merId = "";
		List<String> string = em.createQuery("select o.merId from Merchants o order by o.merId desc").setMaxResults(1).getResultList();
		if(string==null||string.isEmpty()){
			merId = "0000001";
		}else{
			merId = string.get(0);
			merId = merId.substring(10, 15);
			Long newmerId = Long.valueOf(merId)+1;
			merId = newmerId.toString();
		}
		while(merId.length()<5){
			merId = "0" + merId;
		}
		merId = "00000001" + merId;
		while(this.validate(merId)){
			merId = merId.substring(10, 15);
			Long newmerId = Long.valueOf(merId)+1;
			merId = newmerId.toString();
			while(merId.length()<5){
	...
	public List<OptionsString> getOption() {
		List<Merchants> merchantsList = em.createQuery("from Merchants o where o.status = 1 order by merId asc").getResultList();
		List<OptionsString> list = new ArrayList<OptionsString>();
		for (Merchants merchants : merchantsList) {
			list.add(new OptionsString(merchants.getMerId(), merchants.getMerName()+"("+merchants.getOrgans().getName()+")"));
		}
		return list;
	}
	...
	public List<OptionsString> getOptionByOrganId(String organId) {
		String sql = "";
		Organs organs = organsService.find(organId);
		if(organs.getParentId().equals("0")){
			sql = "from Merchants o where o.status = 1 and (o.organs.organId = '"+organId+"' or o.organs.parentId = '"+organId+"') order by merId asc";
		}else{
	...
	public Merchants saveMerchant(MerchantsDTO merchantsDTO) {
		...
		case 0:
			merchants.setOrgans(organsService.find(merchantsDTO.getOrganId()));
			break;
		case 1:
			Organs organs = organsService.find(us.getOrganId());
			if("0".equals(organs.getParentId())
					||organs.getParentId()=="0"){
				merchants.setOrgans(organs);
			}
			else{
				merchants.setOrgans(organsService.find(organs.getParentId()));
			}
			break;
	...
	public boolean validate(String merId) {
		long count = (Long) em.createQuery("select count(o) from Merchants o where o.merId=?1").setParameter(1, merId).getSingleResult();
		return count > 0;
	}

	public boolean validateName(String nameChinese,String merId) {
		long count = (Long) em.createQuery("select count(o) from Merchants o where o.merName ='"+nameChinese+"' and o.merId <> '"+merId+"'").getSingleResult();
		return count > 0;
	}

	@SuppressWarnings("unchecked")
	public QueryResult queryMerByCond(int firstindex, int maxresult,
		...
		case 1:
			if(!us.getPreOrganId().equals("0")){
				sql.append(" and (o.organs.parentId = '"+us.getOrganId()+"' or o.organs.organId = '"+us.getOrganId()+"')");
			}else{
		...
		//商户领卡:如果商户所属机构号不为空，则：
		if(merchantsDTO.getStockSsOrganId()!=null
				&&!"".equals(merchantsDTO.getStockSsOrganId())){
	 ...
	 *@author:井建国
	 *@thorws:
	 *@father:@see com.paySystem.ic.dao.base.MerchantsDao#queryByName(java.lang.String)
	 */
	@SuppressWarnings("unchecked")
