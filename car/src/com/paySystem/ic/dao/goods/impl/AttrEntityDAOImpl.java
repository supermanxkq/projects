...
public class AttrEntityDAOImpl extends DaoSupport<GoodsAttribute> implements
		AttrEntityDAO {
	/**
	 *@Title:findAll
	 ...
	 */
	public QueryResult<GoodsAttribute> findAll(int firstindex, int pageNum,
		...
		if (goodsAttributeDTO.getAttrName() != null
		...
		try {
			/** 获取数据库中所有属性记录 */
			gQueryResult = this.getScrollData(firstindex, pageNum, stringBuffer
	...
	public QueryResult<AttrEntityDTO> showAttrEntities(int firstindex,
		...
		try {
			/** 获取数据库中所有属性记录 */
			gQueryResult = queryAttrEntities(firstindex, pageNum, stringBuffer
					.toString(), params.toArray(), orderBy);
		} catch (Exception e) {
			e.printStackTrace();
		}
		List<AttrEntityDTO> attrEntityDTOs = changeEntityToDTO(gQueryResult
				.getResultlist());
	...
	public QueryResult<AttrEntity> queryAttrEntities(int firstindex,
			int maxresult, String wherejpql, Object[] queryParams,
			LinkedHashMap<String, String> orderby) throws Exception {
		QueryResult<AttrEntity> qr = new QueryResult<AttrEntity>();
		Query query = em.createQuery("select o from  AttrEntity"
				+ " o "
		...
		query = em.createQuery("select count(o.attrEnId) from AttrEntity"
				+ " o "
				+ (wherejpql == null || "".equals(wherejpql.trim()) ? ""
						: "where 1=1 " + wherejpql));
		setQueryParams(query, queryParams);
