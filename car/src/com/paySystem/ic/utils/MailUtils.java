...
public class MailUtils {
     ...
     *@throws:
     */
    public static void sendMail(MailServParamDTO mailServParamDto, String subject, String content, String toMail)
            throws Exception {
        ...
        /** 设定mail server **/
        senderimpl.setHost(mailServParamDto.getEspUrl());

        /** 设置邮箱的登录帐号及密码 **/
        senderimpl.setUsername(mailServParamDto.getEmaccNo());
        senderimpl.setPassword(mailServParamDto.getEmpwd());
        ...
        mp.addBodyPart(bp1);
        mimeMessage.setContent(mp);
        mimeMessage.setFrom(new InternetAddress(senderimpl.getUsername()));
        mimeMessage.setRecipients(Message.RecipientType.TO, InternetAddress.parse(toMail));
        mimeMessage.saveChanges();
        try {
            Transport transport = session.getTransport("smtp");
            transport.connect(senderimpl.getHost(), senderimpl.getUsername(), senderimpl.getPassword());
            transport.sendMessage(mimeMessage, mimeMessage.getRecipients(Message.RecipientType.TO));
            transport.close();
        }
        catch (Exception e) {
