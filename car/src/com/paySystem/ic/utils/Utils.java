...
public class Utils {
	 ...
	 * 异步输出字节流
	 * 对比上面的方法的区别在于头不一样。
	 * 用于解决ajaxfileupload在chrome下面的兼容性问题
	 ...
	 * @return
	 */
    public static String getOptionsIntegerName(List<OptionsInteger> list , Integer id){
	 ...
	 * @return
	 */
    public static String getOptionsStringName(List<OptionsString> list , String id){
     ...
     * @return
     */
    public static String getSelectHtml(List<OptionsInteger> OptionsIntegers,String selectName,String selectId,Boolean initFlag){
    	StringBuffer html = new StringBuffer();
    	html.append("<select name='"+selectName+"' id='"+selectId+"' class='formInput'>");
		if(initFlag){
		...
		if(OptionsIntegers!=null&&!OptionsIntegers.isEmpty()){
			for (OptionsInteger optionsInteger : OptionsIntegers) {
				html.append("<option value='"+optionsInteger.getKey()+"'>"+optionsInteger.getValue()+"</option>");
			}
		}
		html.append("</select>");
     ...
     * @return
     */
    public static String getSelectStringHtml(List<OptionsString> optionsStrings,String selectName,String selectId,String value){
    	StringBuffer html = new StringBuffer();
    	html.append("<select name='"+selectName+"' id='"+selectId+"' class='formInput'>");
		html.append("<option value='-1'>请选择</option>");
		if(optionsStrings!=null&&!optionsStrings.isEmpty()){
			for (OptionsString optionsString : optionsStrings) {
				if(optionsString.getKey().equals(value)){
					html.append("<option value='"+optionsString.getKey()+"' selected='selected'>"+optionsString.getValue()+"</option>");
				}else{
					html.append("<option value='"+optionsString.getKey()+"'>"+optionsString.getValue()+"</option>");
				}
			}
		}
		html.append("</select>");
	...
	public static String enCode(String string){
    	try {
			return StringUtils.isNotBlank(string)?java.net.URLEncoder.encode(string.trim(),"utf-8"):"";
	...
	public static String deCode(String string){
    	try {
			return StringUtils.isNotBlank(string)?java.net.URLDecoder.decode(string.trim(),"utf-8"):"";
		} catch (UnsupportedEncodingException e) {
			e.printStackTrace();
		} 
		return null;
	...
	public static String transCoding(String paramName){
		...
		try {
			if(ServletActionContext.getRequest().getParameter(paramName)!=null){
				result = new String(ServletActionContext.getRequest().getParameter(paramName).getBytes("iso-8859-1"), "utf-8");
			}
		} catch (UnsupportedEncodingException e) {
			e.printStackTrace();
		}
	...
	public static Boolean checkPermission(String privilegeId){
    	if(Globals.IS_VERIFY){
    		Set<String> privileges = (Set<String>) ServletActionContext.getRequest().getSession().getAttribute(Globals.USER_PRIVILEGES);
	...
	public static UserSession getUserSession(){
		UserSession us = (UserSession) ServletActionContext.getRequest().getSession().getAttribute(Globals.USER_SESSION);
		return us;
	}
	
	/**
	 * UKEY验证
	 * @param keyFlag 密钥标志 1普通密钥 2代理商密钥
	 * @param rnd 随机数
	 * @param return_EncData 客户端加密后的值
	 * @param keyID 客户端keyID
	 * @param userKeyID 登陆用户数据库keyID
	 * @return
	 */
	@SuppressWarnings("static-access")
	public static ReturnDTO checkUKEY(Integer keyFlag, String rnd,String return_EncData,String keyID,String userKeyID){
		...
		try {
			if(Globals.IS_VERIFY_UKEY){
				...
				if (j9.get_LastError() != 0){ 
					...
					log.info("rnd="+rnd+",return_EncData="+return_EncData+",keyID="+keyID+",userKeyID="+userKeyID);
					log.info("j9.EncString(rnd, DevicePath) = "+outString+"【EncString错误码】"+j9.get_LastError());
					return dto;
				}
	...
	public static String inversion(String num){
		...
		for(int j = 0;j<num.length();j++){
			...
			result = result +Integer.toHexString(Integer.valueOf(single)^0X0F);
		}
	 ...
	 * 获取读卡号密码
	 * @param seriNo 序列号
	 * @param readCardNoKey 读卡号密钥
	 * @return
	 * @throws Exception
	 */
	public static String getReadCardNoKey(String seriNo,String readCardNoKey) throws Exception{
		//密钥
		String key = Globals.M1CARD_KEY;
		//读卡号密钥解密
	...
	/**
	 * 获取写卡号密码
	 * @param writeCardNoKey 写卡号密钥
	 * @return
	 * @throws Exception
	 */
	public static String getWriteCardNoPwd(String writeCardNoKey) throws Exception{
		//密钥
		String key = Globals.M1CARD_KEY;
    ...
    */
	...
	@SuppressWarnings("unchecked")
	public static Map<String, String> processParameterMap(HttpServletRequest request){
	...
	public static void main(String[] args) throws Exception {
		...
		System.out.println(Utils.deCode("%D6%D0%B9%FA%B6%CC%D0%C5%CD%F8%B7%A2%CB%CD%B2%E2%CA%D4"));
	...
	/**
	 * 自动生成ID流水号方法
	 * @param em EntityManager                        必填项 
	 * @param id 要生成流水号的属性名称，如：termId     必填项
	 * @param entityName  要生成流水号号的实体类，如：Terminals  必填项
	 * @param maxLength 该列的长度                                                       必填项
	 * @param midIndex  要在第几位插入字符                                   不需要则传 0
	 * @param midString 要在中间插入的字符串
	 * @param prefix    流水号前缀
	 * @param suffix    流水号后缀
	 * @return id
	 */
	@SuppressWarnings("unchecked")
	public static String createSerialNum(EntityManager em,
			                             String id,
			                             String entityName,
			                             int maxLength,
			                             int midIndex,
		...
		String validateSql = "select count(o) from "+entityName+" o where o."+id+"=?1";
		/*long count = (Long) em.createQuery(validateSql).setParameter(1, id).getSingleResult();
		boolean flag = (count>0);*/
		String createId = "";
		String tempString = "";
		String querySql = "select o. "+id+" from "+entityName+" o order by o."+id+" desc";
		List<String> string = em.createQuery(querySql).setMaxResults(1).getResultList();
		if(string==null||string.isEmpty()){
			...
			String tempStr = createId.substring(prefixLength,(createId.length()-suffixLength));
		...
		while((Long) em.createQuery(validateSql).setParameter(1, tempString).getSingleResult()>0){
		...
		if(StringUtils.isNotBlank(midString)&&midIndex!=0){
			...
			String str2 = tempString.substring(midIndex+midStrLen-1, tempString.length());
