...
public class SoftKey2 {
    ...
    public String  StrEnc(String InString , String Key)//使用增强算法，用软件加密字符串
    {
    ...
    public String  StrDec(String InString )//使用增强算法，用锁解密字符串,
    {
        ...
        for( n = 0; n<=outlen - 8 ;n=n+ 8)
        {
            for(i=0;i<8;i++)
            {
            	jsyunew6.SetEncBuf(n, b[i + n]);
            }
            int ret = jsyunew6.Cal_New(DevicePath);
            if (ret != 0) { System.out.println("加密数据时失败"); return ""; }
            for(i=0;i<8;i++)
            {
            	outb[i + n]=(byte)jsyunew6.GetEncBuf(n);
	...
	{
		SoftKey2 mysoftkey = new SoftKey2();
