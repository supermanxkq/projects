...
<meta http-equiv="Content-Type" content="text/html; charset=UTF-8">
	...
	<script type="text/javascript">
	    ...
	    function queryCardNo(){
	        ...
	        var binId =$("#binId").val();
			if(!reg.test(cardNoView)){
				$("#cardNoViewValue").html("卡号必须全为数字!");
				return;
			}
			...
			var actionUrl = "account/accFreeze!queryCardNo";   
		    $.ajax( {   
		        url : actionUrl,   
		        data : params,   
		        dataType : "json",
        ...
        var ajaxTypes =function(){
			...
			var actionUrl = "account/accFreeze!ajaxTypes";   
		    $.ajax({
	        url : actionUrl,
			data : params,
			dataType : "json",
			...
			success : function(data, textStatus) {
             if(data.flag){
                 ...
                 $("#typeIdErrorMsg").html("该账户已冻结或账户类型不存在");
                 $("#flag1").val("0");
              }
			}
		});
	...
	/**删除不是数字的字符**/
	 function replaceToNum(obj){
		 if(isNaN(obj.value)){
			 	obj.value = obj.value.replace(/[\D]/ig,"");   //清除“数字”和“.”以外的字符
			 }
	 }
...
<body onload="loadset();">
	...
	<s:form action="account/accFreeze" method="post" onsubmit="document.getElementById('submitInput').disabled = true;return true;" theme="simple">
		  	...
		  	<tr>
					...
					<td colspan="3">
						<s:textarea name="accRecordDTO.descApp" id="descApp"
							cssClass="formTextarea" cols="45" rows="20" style="resize:none;	" maxlength="240"  />
					</td>
				</tr>
		</table>
	...
	<table id="cardInfoTb" class="listTable" width="100%">
	 ...
	 <div class="formTableBottom">
		...
		<input type="button" class="formButton" value="返 回" onclick="go('account/accFreeze!list')"/>
	 </div>
	 </s:form>
</body>
</html>