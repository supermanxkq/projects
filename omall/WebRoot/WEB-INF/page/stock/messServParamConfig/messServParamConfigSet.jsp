...
<html>
    ...
    /**验证全部*/
     function check(){
    	 ...
    	 checkMobile1();
    	 checkContent();
    	 if(servUrlValueFlag&&AccountFlag&&AccountValueFlag&&passwordErrorFlag&&passwordValueErrorFlag&&servUrlValueFlag1&&AccountFlag1&&AccountValueFlag1&&passwordErrorFlag1&&passwordValueErrorFlag1&&mobileFlag1&&mobileFlag&&contentFlag){
						return true;
          	}else{
              			alert("*号为必填项！");
						return false;
	...
	<body>
		...
		<div class="search" id="tabs">
			...
			<s:form action="stock/messServParamConfig" method="post"
				...
				<s:if test="method=='updateData'">
					...
					<s:if test="messServParamConfigDTO.paramType==1">
						<div class="search" id="tabs-1">
							<jsp:include page="shortMessageSet.jsp"></jsp:include>
						</div>
					</s:if>
					<s:else>
						<div class="search" id="tabs-2">
							<jsp:include page="verificationCodeSet.jsp"></jsp:include>
						</div>
					</s:else>
				</s:if>
				<s:else>
				...
				<div class="formTableBottom">
					<s:if test="method=='addSave'">
						<my:permission key='sy-6701-01' value='短信服务器配置信息添加'>
					...
					<s:elseif test="messServParamConfigDTO.paramType1==2">
						<my:permission key='sy-6701-03' value='短信服务器配置信息修改'>
							<input id="submitInput" type="submit" class="formButton"
