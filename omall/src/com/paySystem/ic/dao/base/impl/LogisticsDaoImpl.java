...
public class LogisticsDaoImpl extends DaoSupport<Logistics> implements LogisticsDao {
	...
	/**
	 * 批量查询物流公司
	 *@Title: batchQueryLogisticsList
	 *@Description:  批量查询物流公司
	 *@param logiIdSet 物流id
	 *@Return: List<LogisticsDTO> 物流公司结果集
	 *@author: Jacky
	 *@Thorws:
	 */
	@SuppressWarnings("unchecked")
	public List<LogisticsDTO> batchQueryLogisticsList(Set<Integer> logiIdSet) {
		List<Logistics> logiList = em.createQuery("from Logistics where logistId in (:idSet)").setParameter("idSet", logiIdSet).getResultList();
		List<LogisticsDTO> logiDTOList = new ArrayList<LogisticsDTO>();
		if(CollectionUtils.isNotEmpty(logiList)){
			for (Logistics logistics : logiList){
				LogisticsDTO logisticsDTO = new LogisticsDTO();
