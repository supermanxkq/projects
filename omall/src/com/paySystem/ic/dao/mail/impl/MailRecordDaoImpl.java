...
public class MailRecordDaoImpl extends DaoSupport<MailRecord> implements MailRecordDao {
    /***
     * 
     *@Title:queryResult
     *@TODO:查询结果
     ...
     *@thorws:
     */
    public QueryResult<MailRecordDTO> queryResult(int fristindex, int pageNum, MailRecordDTO mailRecordDTO,
        ...
        if (StringUtils.isNotBlank(mailRecordDTO.getSendDate())) {
            sql.append("and str_to_date(date_format(o.sendDate,'%Y-%m-%d'),'%Y-%m-%d')= str_to_date('"
                    + mailRecordDTO.getSendDate() + "','%Y-%m-%d')");
        }
        QueryResult<MailRecord> queryResult = getScrollData(fristindex, pageNum, sql.toString(), params.toArray(),
                orderby);
        List<MailRecordDTO> list = getMailRecordList(queryResult.getResultlist());
