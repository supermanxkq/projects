	...
	public String right() {
        ...
        if(us.getUserLevel()==2){
       	 try {
				...
				homePageShowDTO.setStoreName(sto.getStoreName());
				homePageShowDTO.setStoreAddress(sto.getStoreAddress());
				homePageShowDTO.setStoreIntroduct(sto.getStoreIntroduct());
				homePageShowDTO.setContPerson(sto.getContPerson());
				homePageShowDTO.setTeleNo(sto.getTeleNo());
				UndealServiceNumDTO undealServiceNumDTO=undealServiceNumService.findList(merId);
				this.getRequest().setAttribute("undealServiceNumDTO", undealServiceNumDTO);
			} catch (Exception e) {
				e.printStackTrace();
			}
        }else{
       	 try {
				...
				homePageShowDTO.setName(org.getName());
				homePageShowDTO.setAddress(org.getAddress());
				homePageShowDTO.setConPerName(org.getConPerName());
				homePageShowDTO.setConPerTeleNo(org.getConPerTeleNo());
				UndealServiceNumDTO undealServiceNumDTO=undealServiceNumService.findTotalData(organId);
				this.getRequest().setAttribute("undealServiceNumDTO", undealServiceNumDTO);
			} catch (Exception e) {
				e.printStackTrace();
			}
