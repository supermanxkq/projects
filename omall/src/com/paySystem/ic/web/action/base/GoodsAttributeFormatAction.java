...
public class GoodsAttributeFormatAction extends BaseAction {
	...
	private static final Log log = LogFactory
			.getLog(GoodsAttributeFormatAction.class);
	...
	public String jsonPageList() throws Exception {
		...
		QueryResult<List> qr = attributeFormatService.getAssociatedFamily(
				(familyDTO.getPage() - 1) * pageNum, pageNum, familyDTO,
				orderby);
		...
		for (int i = 0; i < familyIds.size(); i++) {
			...
			List<GoodsAttributeDTO> attributeDTOs = attributeFormatService
					.getAttributeByFamilyId(familyId, "list");
			attrNames = "";
			...
			if (Utils.checkPermission("sy-1704-04")) {
				operation += "<a href=javascript:showFormatByFamilyId('base/goodsattributeformat!getFormatByFamilyId','"
						+ familyId
						+ "')"
						+ " title='相关规格'>"
						+ Globals.IMG_VIEW
		...
		PageView pageView = new PageView(familyDTO.getPage(), qr
				.getTotalrecord());
	...
	public String addUI() throws Exception {
		...
		this.getRequest().setAttribute("status",
				OptionsValue.STATUS_ENABLE_DISABLE);
		// 获取类型的第一级分类
	...
	public String addSave() {
		...
		for (int i = 0; familyAttrRelaDTOs != null
				&& i < familyAttrRelaDTOs.size(); i++) {
		...
		for (int i = 0; familyGroupRelaDTOs != null
				&& i < familyGroupRelaDTOs.size(); i++) {
		...
		attributeFormatService.saveAssociation(familyDTO.getFamilyId(),
				groupRelaDTOs, attrRelaDTOs);
		/** 2.保存操作日志信息 */
	...
	public String delete() {
		try {
			attributeFormatService.updateStautsByFamilyId(Integer
					.parseInt(this.id), 9);
	...
	public String getAssociationByFamilyId() throws Exception {
		...
		List<GoodsAttributeDTO> attributeDTOs = attributeFormatService
				.getAttributeByFamilyId(familyDTO.getFamilyId(), type);
		if (attributeDTOs == null || attributeDTOs.size() == 0) {
			return Utils.printInfo(zMap);
		}
		List<GoodsFormatDTO> formatDTOs = attributeFormatService
				.getFormatByFamilyId(familyDTO.getFamilyId(), type);
		zMap.put("attributes", attributeDTOs);
		ArrayList groupIds = new ArrayList();
		List<GoodsFormatGroupRelaDTO> dtoResult = attributeFormatService
				.getAllGoodsFormatGroupRela();
		...
		for (int j = 0; j < formatDTOs.size(); j++) {
			...
			for (int i = 0; i < dtoResult.size(); i++) {
				...
				if ((int) formatDTO.getFormatId() == formatId) {
					ArrayList<GoodsFormatDTO> formatList = (ArrayList<GoodsFormatDTO>) zMap
							.get(groupId);
					if (formatList == null) {
	...
	public String getFormatByFamilyId() throws Exception {
		List<GoodsFormatDTO> formatDTOs = attributeFormatService
				.getFormatByFamilyId(familyDTO.getFamilyId(), "list");
		HashMap zMap = new HashMap();
		if (formatDTOs == null) {
			return Utils.printInfo(zMap);
		}
	 ...
	 *@Return:String
	 *@author:王楠
	 *@Date:2014-12-1上午11:40:02
	 */
	public String editAttribute() {
		ReturnDTO data = new ReturnDTO();
		attributeFormatService.editAttribute(attributeDTO);
		functionsService.saveFunction("商品分类属性规格管理", 2, "修改属性：");
		data.setFlag(true);
		return Utils.printInfo(data);
	}
	...
	public String getAllAttributes() throws Exception {
		QueryResult<GoodsAttributeDTO> dtoResult = attributeFormatService
				.getAllAttributes();
	...
	public String getAllFormatGroups() throws Exception {
		QueryResult<GoodsFormatGroupDTO> dtoResult = attributeFormatService
				.getAllFormatGruops();
	...
	public String getFormatsByGroup() throws Exception {
		List list = attributeFormatService.getFormatsByGroup(formatGroupRelaDTO
				.getFgroupId());
		List<List<String>> lists = new ArrayList<List<String>>();
	...
	public String checkAttributeName() throws Exception {
		ReturnDTO data = new ReturnDTO();
		boolean flag = attributeFormatService.checkAttributeName(attributeDTO
				.getAttrName());
		data.setFlag(flag);
		return Utils.printInfo(data);
	}
	...
	public String checkDisplayName() throws Exception {
		ReturnDTO data = new ReturnDTO();
		boolean flag = attributeFormatService.checkDisplayName(attributeDTO
				.getShowlable());
		data.setFlag(flag);
		return Utils.printInfo(data);
	}
	...
	public String checkFormatName() throws Exception {
		ReturnDTO data = new ReturnDTO();
		boolean flag = attributeFormatService.checkFormatName(formatDTO
				.getFormatName(), formatGroupDTO.getFgroupId());
		data.setFlag(flag);
		return Utils.printInfo(data);
	}
	...
	public String checkFormatGroupName() throws Exception {
		ReturnDTO data = new ReturnDTO();
		boolean flag = attributeFormatService
				.checkFormatGroupName(formatGroupDTO.getFgroupName());
		data.setFlag(flag);
		return Utils.printInfo(data);
	}
	...
	public String export() {
		...
		File file = new File(path + "/upload/excel/导入商品规格模板.xls");
		...
		// 封装商品规格
		List<GoodsFormatDTO> goodsFormatList = new ArrayList<GoodsFormatDTO>();
		GoodsFormatDTO goodsFormat = null;
		// 封装商品规格分组
		List<GoodsFormatGroupDTO> goodsFormatGroupList = new ArrayList<GoodsFormatGroupDTO>();
		GoodsFormatGroupDTO goodsFormatGroup = null;
		for (int i = 0; i < lists.size(); i++) {
			Object obj = lists.get(i);

			if (null == obj) {
				log
						.error("Error! GoodsAttributeFormatAction.Export() encounted an error! Data reading exception!");
				dto.setMsg("error");
				return Utils.printInfo(dto);
			}
			...
			if (str.length != 2 && str.length != 3) {
				// 标题行不读取数据，显示时要加上
				log.info((i + 2) + " | Successfully Export " + i
						+ " line data!");
				break;
			}

			// 封装商品规格
			goodsFormat = new GoodsFormatDTO();
			goodsFormat.setFormatName(Utils.getString(str[0]));
			goodsFormatList.add(goodsFormat);

			String isGroup = Utils.getString(str[1]);
			goodsFormatGroup = new GoodsFormatGroupDTO();
			if ("是".equals(isGroup)) {
				goodsFormatGroup.setFgroupName(Utils.getString(str[2]));
			}
			goodsFormatGroupList.add(goodsFormatGroup);
		}

		try {
			GoodsFormatDTO format = null;
			GoodsFormatGroupDTO formatGroup = null;
			for (int i = 0; i < goodsFormatList.size(); i++) {
				format = goodsFormatList.get(i);
				formatGroup = goodsFormatGroupList.get(i);
				attributeFormatService.saveForamtAndFGroup(format, formatGroup);
				dto.setMsg("success");
				// 上传成功后删除文件
				if (file.exists()) {
					file.delete();
			...
			log
					.error("GoodsAttributeFormatAction.export() encounted an error!");
			dto.setMsg("error");
		}
		return Utils.printInfo(dto);
	}
