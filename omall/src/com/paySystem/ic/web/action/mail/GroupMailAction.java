...
public class GroupMailAction extends BaseAction {
	...
	public String list() {
		memGroupsDTO.setBeginDate(DateTimeTool.dateFormat("yyyy-MM-dd",DateTimeTool.nDaysAgo(7, new Date())));
		memGroupsDTO.setEndDate(DateTimeTool.dateFormat("yyyy-MM-dd",new Date()));
	...
	public String jsonPageList() throws Exception {
		...
		QueryResult<MemGroups> queryResult = memGroupsService.queryResult(
		...
		for (int i = 0; i < list.size(); i++) {
			...
			strings.add("<input type='checkbox' name='email' title='"
					+ m.getGroupName() + "' value='" + m.getGroupId()
					+ "' onclick='check();' /> ");
			...
			strings.add(DateTimeTool
					.dateFormat("yyyy-MM-dd", m.getCreateTime()));
			strings.add(Utils.getString(memberGroupsDetailService
					.findByGroupId(m.getGroupId()).size()));
	...
	public void sendMail() throws Exception {
		...
		if (emailArr != null) {
			if (emailArr.length == 1) {
				groupList = memberGroupsDetailService.findByGroupId(Integer
						.parseInt(emailArr[0]));
	...
	public MailRecordDTO getMailRecordDTO() throws Exception {
		...
		mailRecordDTO.setSendDate(DateTimeTool.dateFormat("yyyy-MM-dd",
				mailContentService.getSysTime()));
	 ...
	 *@thorws:
	 */
	public String[] getMail(List<List<Members>> listt) {
		String[] groupMail = null;
	 ...
	 *@thorws:
	 */
	public String[] getMailArr(List<Members> list) {
		String[] groupMail = null;
		if (list != null) {
			if (list.size() == 1) {
