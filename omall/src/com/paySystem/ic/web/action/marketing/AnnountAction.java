...
public class AnnountAction extends BaseAction {
	 ...
	 */
	public String list() {
		annountDTO.setCreateTime(DateTimeTool.dateFormat("yyyy-MM-dd",
				DateTimeTool.nDaysAgo(7, new Date())));
		annountDTO.setStopTime(DateTimeTool
				.dateFormat("yyyy-MM-dd", new Date()));
		this.getRequest()
				.setAttribute("statusList", OptionsValue.Announ_STATUS);
	...
	public String jsonPageList() {
		...
		try {
			result = annountService.queryResult((annountDTO.getPage() - 1)
					* pageNum, pageNum, annountDTO, orderby);
		} catch (Exception e) {
		...
		for (int i = 0; i < listDto.size(); i++) {
			...
			dto.setReportTime(DateTimeTool.dateFormat("yyyy-MM-dd HH:mm:ss",
					DateTimeTool.dateFormat("yyyy-MM-dd HH:mm:ss", dto
							.getReportTime())));
			...
			if (dto.getStatus() == 0) {
				...
				if (Utils.checkPermission("sy-7101-03")) {
					operation += "<a href =marketing/annount!editUI?annountDTO.AnnounId="
							+ dto.getAnnounId()
							+ " title='修改'>"
							+ Globals.IMG_EDIT + "</a>&nbsp;";
				}
				...
				if (Utils.checkPermission("sy-7101-04")) {
					operation += "<a href=javascript:deleteData('marketing/annount!delete','"
							+ dto.getAnnounId()
							+ "') title='删除'>"
							+ Globals.IMG_DELETE + "</a>&nbsp;";
				}
				if (Utils.checkPermission("sy-7101-01")) {
					operation += "<a href =marketing/annount!checkUI?annountDTO.AnnounId="
							+ dto.getAnnounId()
							+ " title='查看'>"
							+ Globals.IMG_VIEW + "</a>&nbsp;";
				}
				if (Utils.checkPermission("sy-7101-05")) {
					operation += "<a href=javascript:reprotAnnount('marketing/annount!reportAnnount','"
							+ dto.getAnnounId()
							+ "') title='发布'>"
							+ Globals.IMG_AUDIT + "</a>&nbsp;";
				...
				if (Utils.checkPermission("sy-7101-06")) {
					operation += "<a href=javascript:CancerReprotAnnount('marketing/annount!cancerReportAnnount','"
							+ dto.getAnnounId()
							+ "') title='取消发布'>"
							+ Globals.IMG_AUDIT + "</a>&nbsp;";
		...
		PageView pageView = new PageView(annountDTO.getPage(), result
				.getTotalrecord());
	...
	public String addUI() {
		...
		this.getRequest().setAttribute("statusList",
				OptionsValue.AddAnnoun_STATUS);
	...
	public String addSave() throws Exception {
		annountDTO.setCreateTime(DateTimeTool.dateFormat("yyyy-MM-dd HH:mm:ss",
				new Date(System.currentTimeMillis())));

		if (annountDTO.getStatus() == 1) {
			annountDTO.setReportTime(DateTimeTool
					.dateFormat("yyyy-MM-dd HH:mm:ss", new Date(System
							.currentTimeMillis())));
		}
		annountService.addSaveAnnount(annountDTO);
		/** 添加功能日志信息 */
		functionsService.saveFunction("全站公告管理", 1, "增加公告："
	...
	public String checkAnnounTitle() {
		...
		/** 设置返回标志信息 */
		dto.setFlag(annountService.findAnnounTitle(announTitle, method,
				announId));
	...
	public String editUI() {
		...
		this.getRequest().setAttribute("statusList",
				OptionsValue.Announ_UpdateSTATUS);
		this.getRequest().setAttribute("isTop", OptionsValue.Announ_IsTop);
		annountDTO = annountService.find(annountDTO.getAnnounId());
		if (annountDTO != null) {
			return INPUT;
		...
		if (annountDTO != null) {
			if (annountDTO.getStatus() == 1) {
				annountDTO.setReportTime(DateTimeTool.dateFormat(
						"yyyy-MM-dd HH:mm:ss", new Date(System
								.currentTimeMillis())));
