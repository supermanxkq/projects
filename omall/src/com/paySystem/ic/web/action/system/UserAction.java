...
public class UserAction extends BaseAction {
	...
	public String jsonPageList() throws Exception {
		...
		//返回结果
		QueryResult<User> queryResult =userService.getScrollData((userDTO.getPage() - 1) * pageNum, pageNum, sb.toString(),params.toArray(), orderby);
		...
		for(int i = 0;i <list.size();i++){
			...
			strings.add(Utils.getOptionsIntegerName(OptionsValue.USER_LEVEL, user.getUserLevel()));
			//strings.add(StringUtils.isNotBlank(user.getKeyID())?"已绑定":"未绑定");
			strings.add(Utils.getOptionsIntegerName(OptionsValue.STATE_STATUSD, user.getStatus()));
			String operation = "";
			if(user.getStatus()!=9){
				if(Utils.checkPermission("sy-9103-03")){
					operation += "<a href=system/user!editUI?userDTO.userName="+user.getUserName()+" title='修改'>"+Globals.IMG_EDIT+"</a>&nbsp;";
					//operation += "<a href=javascript:loadData('"+user.getUserName()+"') title='修改'>"+Globals.IMG_EDIT+"</a>&nbsp;";
				}
				if(Utils.checkPermission("sy-9103-04")){
					operation += "<a href=javascript:deleteData('system/user!delete','"+user.getUserName()+"') title='删除'>"+Globals.IMG_DELETE+"</a>&nbsp;";
				}
				if(Utils.checkPermission("sy-9103-05")){
					operation += "<a href=javascript:initData('"+user.getUserName()+"') title='初始化密码'><img src='images/cx1.gif'/></a>&nbsp;";
				}
				if(Utils.checkPermission("sy-9103-06")){
					if(StringUtils.isBlank(user.getKeyID())){
						operation += "<a href=javascript:bangUKeyUI('"+user.getUserName()+"') title='绑定UKEY'><img src='images/lock.gif'/></a>&nbsp;";
					}else{
						operation += "<a href=javascript:tieUKey('"+user.getUserName()+"') title='解绑UKEY'><img src='images/unlock.gif'/></a>&nbsp;";
					}
		...
		PageView pageView = new PageView(userDTO.getPage(),queryResult.getTotalrecord());
	...
	public String ajaxRole(){
		...
		if(userDTO.getUserLevel()==0){
			list = roleService.findByJpl("from Role o where o.status = 1 and (o.organId = '"+ReadInit.read("ORGANID")+"' or o.isCommon = 1)");
		}else if(userDTO.getUserLevel()==1){
			list = roleService.findByJpl("from Role o where o.status = 1 and (o.organId = '"+userDTO.getOrganId()+"' or o.isCommon = 1)");
		}else{
			list = roleService.findByJpl("from Role o where o.status = 1 and (o.organId = '"+merchantsService.find(userDTO.getMerId()).getOrgans().getOrganId()+"' or o.isCommon = 1)");
		}
		...
		for(int i = 0;i<list.size();i++){
			...
			checkbox += "<td width='20%'><input type='checkbox' name='userDTO.roleIds' id='dialog_role_"+role.getId()+"' value='"+role.getId()+"' ";
			if(roleSet!=null&&roleSet.contains(role)){
				checkbox += "checked='checked'";
			}
			checkbox += " />";
			checkbox += "<label for='dialog_role_"+role.getId()+"' class='checkboxLabel'>"+role.getName()+"</label></td>";
			if(i%3==-1){
				checkbox += "</tr>";
			}
		}
	...
	public String addUI(){
		...
		switch (us.getUserLevel()) {
		case 0:
			this.getRequest().setAttribute("userLevelValues",OptionsValue.USER_LEVEL );//用户等级
			this.getRequest().setAttribute("organValues", organsService.getOption());
			this.getRequest().setAttribute("merchantValues", merchantsService.getOption());
			this.getRequest().setAttribute("redpassOrganName", ReadInit.read("NAME"));
			break;
		case 1:
			userDTO.setUserLevel(1);
			userDTO.setOrganId(us.getOrganId());
			userDTO.setOrganName(us.getOrganName());
			this.getRequest().setAttribute("userLevelValues",OptionsValue.USER_LEVEL1 );//用户等级
			this.getRequest().setAttribute("merchantValues", merchantsService.getOptionByOrganId(us.getOrganId()));
			break;
		case 2:
			userDTO.setUserLevel(2);
			userDTO.setMerId(us.getMerId());
			userDTO.setMerName(us.getMerName());
			this.getRequest().setAttribute("userLevelValues",OptionsValue.USER_LEVEL2 );//用户等级
			break;
		}
		return INPUT;
	}
	...
	public String addSave() throws Exception {
		if (userService.exist(userDTO.getUserName())) {
			this.getRequest().setAttribute("result",this.getText("exist.username.notice"));
		...
		case 0:
			user.setOrganId(ReadInit.read("ORGANID"));
			break;
		case 1:
		...
		if(userDTO.getRoleIds()!=null){
			for (String string : userDTO.getRoleIds()) {
				if(StringUtils.isNotBlank(string)){
					user.getRoles().add(roleService.find(Integer.valueOf(string)));
				}
			}
		}
	...
	public String editUI() {
		...
		User user = null;
		try {
			user = userService.find(new String(userDTO.getUserName().getBytes("ISO-8859-1"),"utf-8"));
		...
		if (user != null) {
			...
			while(iterator.hasNext()){
				...
				roleNameString.append(role.getName());
			}
			
			
			if(roleNameString.toString().indexOf("系统管理员")==-1){
				if(user.getUserName().equals(us.getUserName())){
					this.getRequest().setAttribute("statusValues",OptionsValue.STATE_STATUSD.get(0) );//状态
				}else{
					this.getRequest().setAttribute("statusValues",OptionsValue.STATE_STATUSD );//状态
				}
			}else{
			...
			userDTO.setLoginFlag(Utils.getOptionsIntegerName(OptionsValue.LOGIN_FLAG, user.getLoginFlag()));
	...
	public String init() {
		...
		try {
			...
			if (user != null) {
				user.setPassWord(MD5.MD5Encode(Globals.PASSWORD));
				userService.update(user);
				functionsService.saveFunction("用户管理", 2, "初始化密码："+user.getUserName());
			}
			this.ajaxResult = "ajaxsuccess";
		} catch (Exception e) {
			e.printStackTrace();
	...
	public String checkKeyID(){
		...
		List<User> list = userService.findByJpl("from User o where o.keyID ='"+userDTO.getKeyID().toUpperCase()+"'");
	...
	public String bangUKey(){
		...
		try {
			...
			if (user != null) {
				...
				userService.update(user);
				functionsService.saveFunction("用户管理", 2, "绑定UKEY："+user.getUserName());
			}
			this.ajaxResult = "ajaxsuccess";
		} catch (Exception e) {
			e.printStackTrace();
	...
	public String tieUKey(){
		...
		try {
			...
			if (user != null) {
				user.setKeyID("");
				userService.update(user);
				functionsService.saveFunction("用户管理", 2, "解绑UKEY："+user.getUserName());
			}
			this.ajaxResult = "ajaxsuccess";
		} catch (Exception e) {
			e.printStackTrace();
